...
import { bootstrapFilePath } from '../ir/derive.js'
import { formatTS } from '../codegen/format.js'
import type { OutputBundle } from './files.js'

/**
 * Emit a starting `demos/src/<project>/index.ts` that boots the project
 * with `RQServer.start(...)`. Uses bundle.setOnce so author edits
 * (real auth, custom appConfig) survive re-runs.
 */
export async function emitBootstrap (spec: ResolvedSpec, bundle: OutputBundle): Promise<void> {
  const source = await formatTS(printBootstrap(spec))
  bundle.setOnce(bootstrapFilePath(spec.project.id), source)
}

function printBootstrap (spec: ResolvedSpec): string {
  return `import { RQServer } from '@reqquest/api'
...
async function main () {
  ...
  await server.start({
    appConfig: {
      multipleRequestsPerPeriod: ${spec.project.multipleRequestsPerPeriod},
      userLookups: {
