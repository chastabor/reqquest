...
For framework background, read `DESIGN.md`. For the spec format reference, read `SPECS.md`. This file is the operator guide — how to write a spec, run the generator, and iterate.
...
The full grammar (every field, hook value, rule key, UI shape) is documented in `SPECS.md`. Use `requirements/demo-default2.spec.yml` as a working reference covering most features.
...
`text:` and `message:` strings support `{{ expression }}` interpolation; `conditional:` / `when:` are full TS expressions. Identifier resolution rules (bare → `data.<field>`, prompt-id → `data.<snake>?.…`) are in SPECS.md §6.5.
...
- **`SPECS.md`** — canonical spec format reference: every field, every hook value, every rule key, every UI shape, every cross-reference rule. Read this when authoring.
